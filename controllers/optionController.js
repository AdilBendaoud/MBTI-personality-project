...
exports.list = async (req, res) => {
  ...
  try {
    ...
    const resultsPromise = Option.find()
        .skip(skip)
        .limit(limit)
        .sort({ created: "desc" })
        .populate('question');
    // Counting the total documents
    const countPromise = Option.count();
...
exports.read = async (req, res) => {
  try {
    // Find document by id
    const tmpResult = await Option.findOne({
      _id: req.params.id,
    });
...
exports.create = async (req, res) => {
    try {
      ...
      const result = await new Option(req.body).save();
      console.log(result);
...
exports.update = async (req, res) => {
  ...
  exports.delete = async (req, res) => {
    try {
      ...
      // Find the document by id and delete it
      const result = await Option.findOneAndDelete({ _id: req.params.id }).exec();
      // If no results found, return document not found
      if (!result) {
        return res.status(404).json({
          success: false,
...
exports.search = async (req, res) => {
    ...
    try {
      let results = await Option.find(fields).sort({ name: "asc" }).limit(10);
      if (results.length >= 1) {
        return res.status(200).json({
          success: true,
          result: results,
